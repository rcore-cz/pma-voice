...
RegisterNetEvent('rcore_dispatch:client:syncGRadio', function(data)
...
RegisterCommand('+radioglobaltalk', function()
    ...
    if not globalRadio then return end
    if not radioPressed and radioEnabled then
        if radioChannel > 0 then
            TriggerServerEvent('rcore_dispatch:server:speakingGlobal')
            logger.info('[radio] Start broadcasting, update targets and notify server.')
            playerTargets(globalRadio or {}, MumbleIsPlayerTalking(PlayerId()) and callData or {})
            TriggerServerEvent('pma-voice:setTalkingOnRadio', true, globalRadio)
            radioPressed = true
...
RegisterCommand('-radiotalk', function()
    if (radioChannel > 0 or radioEnabled) and radioPressed then
        ...
        if globalRadio and usingGlobalRadio then
            TriggerServerEvent('rcore_dispatch:server:cancelSpeakingGlobal')
            TriggerServerEvent('pma-voice:setTalkingOnRadio', false, globalRadio, true)
